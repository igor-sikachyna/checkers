...
func handlePlayerInfoChannel(ctx context.Context, k keeper.Keeper,
	playerInfoChannel <-chan playerInfoTuple,
	done chan<- error) {
...
func MapStoredGamesReduceToPlayerInfo(ctx context.Context, k keeper.Keeper, chunk uint64) error {
	...
	go loadStoredGames(ctx, k, gamesChannel, chunk)
